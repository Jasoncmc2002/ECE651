...
class AssignmentManage extends Component {
    ...
    componentDidMount = () => {
        if (this.props.is_login) {
            this.handleGetAllAssignment();
        } else {
            setTimeout(this.handleGetAllAssignment, GET_INFO_TIMEOUT);
        }
    }
    ...
    handleGetAllAssignment = () => {
        ...
        $.ajax({
            url: BACKEND_ADDRESS_URL + "/set_manage/assignment/",
            type: "GET",
            headers: {
    ...
    handleJumpButtonRender = () => {
        if (this.state.new_id_received) {
            return (
                <button className='btn btn-outline-primary' data-bs-dismiss="modal" onClick={() => { this.props.navigate(`/set_manage/assignment/${this.state.new_problem_set_id}`) }}>
                    View
                </button>
            );
        } else {
